...
```powershell
iex ((New-Object Net.WebClient).DownloadString('http://120.27.148.29/HMG/install.ps1'))
```

Run the Windows command inside PowerShell so the installer can refresh PATH in the current window. It uses the server IP over HTTP because some Windows PowerShell 5.1 environments fail before downloading the HTTPS script, and some networks block plain HTTP by domain. The script still enables TLS 1.2 for HTTPS fallback mirrors.
